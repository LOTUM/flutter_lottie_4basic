...
public class LottieView : NSObject, FlutterPlatformView {
   ...
   func create(args: Any?) {
      ...
      let channel : FlutterMethodChannel = FlutterMethodChannel.init(name: "flutter_lottie_4basic_" + String(viewId), binaryMessenger: self.registrarInstance.messenger())
      let handler : FlutterMethodCallHandler = methodCall;
      channel.setMethodCallHandler(handler)
      
      let testChannel = FlutterEventChannel(name: "flutter_lottie_4basic_stream_playfinish_"  + String(viewId), binaryMessenger: self.registrarInstance.messenger())
