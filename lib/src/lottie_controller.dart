...
class LottieController {
  ...
  LottieController(int id) {
    this.id = id;
    // print('Creating Method Channel shotouch/flutter_lottie_$id');
    _channel = new MethodChannel('shotouch/flutter_lottie_$id');
    _playFinished = EventChannel('shotouch/flutter_lottie_stream_playfinish_$id');
  }
  ...
  Future<void> playWithProgress({double? fromProgress, required double toProgress}) async {
  ...
  Stream<bool> get onPlayFinished {
    var animationFinished = _playFinished.receiveBroadcastStream().map<bool>((element) => element);
